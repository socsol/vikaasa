...
%     and
%   - minimises the provided cost function, when evaluated each
%   provided state-space point, and summed.
%
% USAGE
%   % Standard use with required arguments:
%   u = CostSumMinFMinCon(x, K, f, c)
%
...
%   u = CostSumMinFMinCon(x, K, f, c, options)
%
% Requires: vk_options
%
% See also: CostMin
...
function u = CostSumMinFMinCon(x, K, f, c, varargin)
    ...
    % as a result.
    loop = @(u) CostSumMinFMinCon_loop(x, u, steps, n, m, next_fn);

    % We want to minimise the cost of the last state.
    cost = @(u) CostSumMinFMinCon_cost(loop(u), u, cost_fn, next_fn, ...
                                       f, m);

    % Non-linear constraints attempt to ensure that control choices stay in the
    % constraint space.
    nonlcon = @(u) CostSumMinFMinCon_nonlcon(loop(u), K);
...
function cost = CostSumMinFMinCon_cost(xs, us, cost_fn, next_fn, f, ...
                                       m)
...
function [cin,ceq] = CostSumMinFMinCon_nonlcon(xs, K)
    n = size(xs, 1);
    p = size(xs, 2);

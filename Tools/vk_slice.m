...
%    - point: the position to do the slice at.
...
function SV = vk_slice_helper(V, slice_axis, plane, distance)
  ...
  % Iterate through all the rows of V.  Copy any that have an axis-th value
  % that is leq than distance-away from plane into NV.
  for row = 1:size(V,1)
